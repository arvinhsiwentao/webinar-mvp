...
|-------|-------------|---------|
| `/` | `src/app/(public)/page.tsx` | Single landing page for webinar ID `1` ("Mike是麦克"). Modal registration. Sections: Hero → Speaker Intro → Schedule/Countdown → Benefits → Urgency. Post-registration redirect threads `email` param to lobby URL for downstream checkout pre-fill. |
