...
## us-stock-course 漏斗（$1 / 1+3 直購）

與模擬直播完全分離的冷流量直購鏈路。賣 $1 的「9 章課程 + 3 天 App VIP」，付款後拿序號去 CMoney 官網登入兌換（把登入摩擦從付款前挪到付款後）。

**路由與檔案：**
- `src/app/(public)/us-stock-course/[angle]/page.tsx` — Server Component，`isValidAngle` 否則 `notFound()`，`generateStaticParams` 預渲染 author/news/feature
- `src/components/us-stock-course/UsStockCourseBody.tsx` — 3 切角共用 body（痛點/4 步框架/9 章/App VIP 對比/直播回放/5 步上手/FAQ）
- `src/components/us-stock-course/IntroVideoPlayer.tsx` — 前導影片播放器，Mux HLS + hls.js + **原生 controls**（非鎖定版 VideoPlayer，行銷片要正常播放）
- `src/app/(public)/us-stock-course/checkout/page.tsx` — 精簡結帳（email gate → Stripe Embedded，固定 `us-stock-1plus3`）
- `src/app/(public)/us-stock-course/checkout/return/page.tsx` — 兌換頁（序號 + cmoney.tw 5 步指引，深色金色）
- `src/lib/usStockCourse.ts` — 單一設定來源：`US_STOCK_ANGLES`、`ANGLE_CONFIG`（per-angle 前導影片 + hook 文案）、`US_STOCK_CONTAINER_WEBINAR_ID`、`US_STOCK_PRODUCT_ID`、`US_STOCK_FUNNEL`

**重用既有機制：** `create-session`（加 `funnel:'us_stock_course'` 參數化 return_url）、`fulfillOrder`、`session-status`、`products.ts`（新增 `us-stock-1plus3` SKU，**不在** `getAllProducts()`）。

**容器 webinar：** 訂單 FK 掛一個隱形 `webinars` row（`US_STOCK_CONTAINER_WEBINAR_ID` env，用 UUID），不出現在任何網址。

**GA4 事件隔離（避免污染 webinar 的 Google Ads Smart Bidding）：** 新漏斗**不發**標準 `purchase`/`begin_checkout`，改發專屬 `c_us_stock_course_begin_checkout` / `c_us_stock_course_add_payment_info` / `c_us_stock_course_purchase`（皆 conversion event，自動帶 gclid/utm）+ `c_us_stock_course_cta_click`；scroll depth `page:'us_stock_course_'+angle`。詳見 `docs/decisions.md` 2026-06-11。

**Phase 0 前提（上線前備齊）：** `STRIPE_PRICE_US_STOCK_1PLUS3`、`US_STOCK_CONTAINER_WEBINAR_ID`、`products.ts` 的 `sheetName/productPackageId/salesCode`（CMoney 兌換序號分頁）、3 支 hook 上 Mux 填 `ANGLE_CONFIG.introVideoHls`、3 個轉換事件在 GA4 標關鍵事件並匯入 $1 廣告帳號。
