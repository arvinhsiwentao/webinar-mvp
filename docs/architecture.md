...
|-------|-------------|---------|
| `/` | `src/app/(public)/page.tsx` | Single landing page for webinar ID `1` ("Mike是麦克"). Modal registration. Sections: Hero → Speaker Intro (hardcoded bio copy) → Schedule/Countdown → Benefits → Urgency. **Hero** uses a `<picture>` element to swap between two pre-designed background images (`/images/hero-mobile.jpg` for <768px, `/images/hero-desktop.jpg` for >=768px) — title/subtitle text is baked into the images, only a CTA button is rendered as an HTML overlay. Post-registration redirect threads `email` param to lobby URL for downstream checkout pre-fill. |
