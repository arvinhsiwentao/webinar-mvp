...
type Device = {
  ...
  label: string;
  hint: string;
  /** Screen-recording tutorial video (Mux HLS). null → show "准备中" placeholder. */
  videoUrl: string | null;
  videoPoster?: string;
  /** Portrait clip (phone screen recording) → taller container, no crop. */
  videoPortrait?: boolean;
  /** Step screenshots (captions baked into each image): {dir}/1.webp … {dir}/{count}.webp */
...
const DEVICES: Device[] = [
  { key: 'computer', label: '电脑', hint: '桌机 / 笔电 / 平板', videoUrl: null, icon: DesktopIcon },
  {
    key: 'phone',
    label: '手机',
    hint: '用手机启用',
...
function VideoSlot({ device }: { device: Device }) {
  if (device.videoUrl) {
    ...
    return (
      <div className={portrait ? 'mx-auto w-full max-w-[320px]' : 'w-full'}>
      ...
      <div className="space-y-5">
        {Array.from({ length: shots.count }, (_, i) => i + 1).map((n) => (
          <Image
            ...
            width={600}
            height={1300}
            className="w-full max-w-[320px] mx-auto h-auto rounded-xl border border-[#E8E5DE] shadow-sm"
          />
        ))}
      </div>
    </div>
...
function QuickLinks() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <a href={COURSE_LINK} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2 rounded-xl bg-white border border-[#E8E5DE] shadow-sm px-6 py-6 hover:border-[#B8953F] transition-colors text-center">
      ...
      <a href={APP_LINK} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2 rounded-xl bg-white border border-[#E8E5DE] shadow-sm px-6 py-6 hover:border-[#B8953F] transition-colors text-center">
    ...
    <div className="min-h-screen bg-[#FAFAF7] text-neutral-700">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 md:py-14">
        ...
        <div className="space-y-8">
          {/* 1) Screen-recording video (or placeholder) */}
          <VideoSlot device={device} />

          {/* 2) Go to activation page — primary action */}
          <ActivationFlow />

          {/* 3) Step-by-step screenshots (captions baked into each image) */}
          {device.shots && <ScreenshotSteps shots={device.shots} />}

          {/* 4) Quick links to course + App */}
          <QuickLinks />
