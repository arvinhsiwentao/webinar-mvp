...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sessionStartTime) {
      ...
      const emailData = confirmationEmail(body.email, body.name, webinar.title, sessionStartTime, liveUrl, speakerAvatarUrl, webinar.evergreen?.timezone, webinar.duration, webinar.id);
      sendEmail(emailData);
    }
